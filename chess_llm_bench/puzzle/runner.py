...
This module handles the execution of puzzle-solving sessions for LLMs.
It manages puzzle presentation, response collection, evaluation, and
detailed performance tracking across different puzzle types.

The runner integrates with the existing chess engine infrastructure
to verify moves and calculate evaluation deltas for comprehensive
puzzle performance analysis.
"""
...
class PuzzleRunner:
    """
    Manages puzzle-solving sessions for LLMs.

    This class orchestrates puzzle presentation to LLMs, collects and validates
    their responses, and maintains detailed performance statistics across
    different puzzle categories and difficulty levels.
    """

    def __init__(self, llm_client: LLMClient, engine: ChessEngine, config: Config):
        """
        Initialize the puzzle runner.
    ...
    async def run_puzzle_session(self,
                                puzzle_set: PuzzleSet,
                                output_dir: Path,
                                time_limit: float = 30.0) -> PuzzleStats:
        """
        Run a complete puzzle-solving session.
        ...
        """
        logger.info(f"Starting puzzle session: {puzzle_set.name} ({len(puzzle_set.puzzles)} puzzles)")
        ...
        for i, puzzle in enumerate(puzzle_set.puzzles):
            ...
            try:
                attempt = await self._solve_puzzle(puzzle, time_limit)
    ...
    async def _solve_puzzle(self, puzzle: PuzzlePosition, time_limit: float) -> PuzzleAttempt:
        """
        Solve a single puzzle with the LLM.
    ...
    def _create_puzzle_prompt(self, puzzle: PuzzlePosition, board: chess.Board) -> str:
        ...
        if puzzle.puzzle_type == PuzzleType.TACTIC:
            ...
            base_prompt += "This is a critical position. Find the move that avoids a major blunder.\n"
        ...
        base_prompt += "\nProvide your answer as a single UCI move (e.g., 'e2e4', 'Nf3', 'O-O').\n"
    ...
    def _extract_move_from_response(self, response: str, board: chess.Board) -> str:
        ...
        # This is more complex but handles moves like "Nf3", "O-O", etc.
    ...
    async def _calculate_evaluation_delta(self, llm_move: str, puzzle: PuzzlePosition, board: chess.Board) -> Optional[float]:
    ...
    async def _save_session_results(self, session_dir: Path, puzzle_set: PuzzleSet,
        ...
        with open(summary_file, 'w') as f:
            f.write(f"Puzzle Session Summary\n")
            f.write(f"=====================\n\n")
        ...
        logger.info(f"Session results saved to {session_dir}")

    async def run_standard_puzzle_suite(self, output_dir: Path) -> Dict[str, PuzzleStats]:
        """
        Run a comprehensive puzzle suite covering all puzzle types.

        Args:
            output_dir: Directory to save results

        Returns:
            Dictionary mapping puzzle set names to their statistics
        """
        results = {}

        # Define standard puzzle sets
        puzzle_sets = [
        ...
        logger.info(f"Running standard puzzle suite: {total_sets} puzzle sets")

        for i, (name, puzzle_type, count, difficulty_range) in enumerate(puzzle_sets):
            logger.info(f"Running puzzle set {i+1}/{total_sets}: {name}")
            ...
            try:
                stats = await self.run_puzzle_session(puzzle_set, output_dir)
                results[name] = stats
        ...
        await self._save_suite_summary(output_dir, results)

        return results

    async def _save_suite_summary(self, output_dir: Path, results: Dict[str, PuzzleStats]) -> None:
        """Save summary of the complete puzzle suite."""
        summary_file = output_dir / f"puzzle_suite_summary_{self._session_id}.txt"

        with open(summary_file, 'w') as f:
            f.write(f"Chess Puzzle Suite Summary\n")
            f.write(f"=========================\n\n")
            f.write(f"Bot: {self.llm.spec.name}\n")
            f.write(f"Timestamp: {datetime.now().isoformat()}\n\n")
            ...
            f.write(f"  Total Solved: {total_correct}/{total_attempts} ({overall_rate*100:.1f}%)\n")
            f.write(f"  Average Response Time: {sum(s.average_response_time for s in results.values()) / len(results):.2f}s\n\n")

            f.write(f"Performance by Puzzle Set:\n")
            for name, stats in results.items():
                f.write(f"  {name}: {stats.correct_solutions}/{stats.total_attempts} "
                       f"({stats.success_rate*100:.1f}%) - {stats.average_response_time:.2f}s avg\n")

        logger.info(f"Puzzle suite summary saved to {summary_file}")


# Convenience functions for easy integration

async def run_puzzle_benchmark(bot_spec: BotSpec, config: Config, output_dir: Path) -> Dict[str, PuzzleStats]:
    """
    Run a complete puzzle benchmark for a single bot.
    ...
    """
    from ..llm.client import LLMClient
    from ..core.engine import ChessEngine, create_engine
    ...
    try:
        ...
        # Create puzzle runner
        runner = PuzzleRunner(llm_client, engine, config)

        # Run standard puzzle suite
        results = await runner.run_standard_puzzle_suite(output_dir)
...
def create_custom_puzzle_set(name: str, puzzle_specs: List[Tuple[str, str, int]]) -> PuzzleSet:
    """
    Create a custom puzzle set from FEN positions.

    Args:
        name: Name for the puzzle set
        puzzle_specs: List of (fen, best_move, difficulty) tuples

    Returns:
        PuzzleSet with custom puzzles
    """
    puzzles = []

    for i, (fen, best_move, difficulty) in enumerate(puzzle_specs):
        puzzle = PuzzlePosition(
            fen=fen,
            puzzle_type=PuzzleType.TACTIC,  # Default to tactic
            difficulty=difficulty,
            best_move=best_move,
            title=f"Custom Puzzle {i+1}",
            description=f"Custom puzzle from position {fen[:20]}..."
        )
        puzzles.append(puzzle)

    return PuzzleSet(
        name=name,
        description=f"Custom puzzle set with {len(puzzles)} positions",
        puzzles=puzzles
    )
